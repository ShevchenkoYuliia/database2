...
SELECT 
    ...
    IPS.page_count
FROM 
    sys.dm_db_index_physical_stats (NULL, NULL, NULL, NULL, 'LIMITED') AS IPS
JOIN 
    sys.indexes AS IDX
    ON IPS.OBJECT_ID = IDX.OBJECT_ID
    AND IPS.index_id = IDX.index_id
WHERE 
    IPS.database_id = DB_ID() -- Поточна база даних
ORDER BY 
    IPS.avg_fragmentation_in_percent DESC;
--Завдання 11
ALTER INDEX IX_ComputerMouse_Price ON ComputerMouse REORGANIZE;
--Завдання 12
ALTER INDEX IX_Customer_Email ON Customer REBUILD;
--Завдання 13
DROP INDEX IX_ComputerMouse_Size_Medium ON ComputerMouse;
--Завдання 16
SELECT
